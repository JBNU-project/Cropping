...
def main():
    ...
    # 겹치는 바운드 박스를 제거하는 코드
    # 기존 0.5에서 1로 변경(대각선으로 향하는 글자들의 경우 영역이 겹치기 때문에 작동하지 않도록 수정)
    ...
    if len(indices) > 0:
        ...
        # 원본 입력 이미지 불러오기
        input_image_path = call_args['inputs']
        original_image = cv2.imread(input_image_path)

        # 잘라낸 이미지를 저장할 출력 디렉토리 만들기
        cropped_dir = "./data/cropping"
        ...
        for idx in indices.flatten():
            ...
            # 좌표 추출 및 이미지 자르기
            x_min, y_min, x_max, y_max = map(int, final_bbox)
            cropped_image = original_image[y_min:y_max, x_min:x_max]

            # 잘라낸 이미지 저장
            cropped_path = os.path.join(cropped_dir, f"cropped_{idx}.jpg")
